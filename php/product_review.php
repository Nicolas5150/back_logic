<?php
  ...
  function attemptComment () {
    ...
    // Validate username has not already made a review on the product.
    $status = usernameDuplicate($formFields);
    if ($status['statusCode'] !== '200') {
      return (returnMessage($status['statusCode'], $status['statusMessage']));
    }
    ...
    $status['statusMessage'] = 'Review Created';
    return (returnMessage($status['statusCode'], $status['statusMessage']));
  }
  ...
  // Check that the username has not already hadded a comment to this item.
  function usernameDuplicate($formFields) {
    $conn = connectToDatabase();
    ...
    $conn->close();
      if ($count > 0) {
        $status['statusCode'] = '406';
        $status['statusMessage'] = 'Username has already made comment';
        return $status;
      }
